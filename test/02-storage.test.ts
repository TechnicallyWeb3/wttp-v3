...
describe("TestStorage", function () {
  ...
  describe("Access Control", function () {
    it("Should verify resource admins correctly", async function () {
      const { testStorage, owner, siteAdmin, publicUser } = await loadFixture(deployTestStorageFixture);
      
      const resourceRole = hre.ethers.keccak256(hre.ethers.toUtf8Bytes("RESOURCE_ADMIN_ROLE"));
      await testStorage.connect(owner).createResourceRole(resourceRole);
      
      // Set up a header with the resource admin role
      const headerInfo = {
        cache: { 
          maxAge: 3600,
          sMaxage: 1800,
      ...
      // Create metadata with this header
      const metadata = {
        mimeType: "0x7570",
        charset: "0x7508",
        encoding: "0x6964",
      ...
      await testStorage.testCreateMetadata("/admin-resource.txt", metadata);
      
      // Grant resource admin role to public user
      await testStorage.connect(siteAdmin).grantRole(resourceRole, publicUser.address);
      
      // Check admin status
      expect(await testStorage.testIsResourceAdmin("/admin-resource.txt", owner.address)).to.be.true; // Owner is site admin
      expect(await testStorage.testIsResourceAdmin("/admin-resource.txt", siteAdmin.address)).to.be.true; // Site admin
      expect(await testStorage.testIsResourceAdmin("/admin-resource.txt", publicUser.address)).to.be.true; // Has resource role
    });
  });
});
